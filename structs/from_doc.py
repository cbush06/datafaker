...
class FromDoc:
    ...
    _validators: dict[str, Callable[[any], bool]] = {
        ...
        'doc': lambda _, val: val != None and isinstance(val, str) and len(val.strip()) > 0,
        'multiply': lambda _, val: val != None and (isinstance(val, int) and val > 0)
    }

    def __init__(self, doc_name: str, from_doc: dict[str, any]):
        ...
        for key in from_doc:
            self._extract_metadata_entry(key, from_doc[key])
    ...
    def _extract_metadata_entry(self, key: str, entry):
        if(key in self._metadata):
            if(self._validators[key](self, entry)):
                ...
                raise ValueError(f"Value for {self._doc_name}._from_doc.{key} is invalid")
        else:
            raise ValueError(f"Metadata entry {self._doc_name}._from_doc.{key} is not recognized")
